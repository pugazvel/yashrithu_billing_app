...
import 'package:retail_bill/backup/google_drive_service.dart';
...
class _PrintWidgetState extends State<PrintWidget> {
  ...
  GoogleDriveService googleDrive = GoogleDriveService.instance;
...
void initBluetooth() async {
  await printer.initPlatformState();
  setState(() {
      _connected = printer.isConnected();
      _devices = printer.devices;
  });
}
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(primary: Colors.brown),
                  onPressed: () {
                    Bill bill = new Bill(id: null, billNumber: 1, dateTime: DateTime.now(), amount: 1, quantity: 1, items: List.empty());
                    printer.billPrint(bill);
                  },
    ...
    if (_device != null) {
      bluetooth.isConnected.then((isConnected) {
        if (isConnected == false) {
          ...
          AppService.instance.setConnectedDevice(_device!);
          // AppService.instance.btConnected = _connected;
        }
      });
    } else {
  ...
  void _disconnect() {
    ...
    AppService.instance.setConnectedDevice(device);
  }
